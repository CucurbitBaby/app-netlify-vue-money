    ...
    <div class="register">
        <section class="form_container">
            <div class="manage_tip">
                <span class="title">cucurbitbaby 在线后台管理系统</span>
            </div>
            <el-form :model="registerUser" :rules="rules" class="registerForm" ref="registerForm" label-width="80px">
                ...
                <el-form-item label="选择身份"  prop="identity" class="identity-error">
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        identity:  [{ required: true, message: '请选择角色', trigger:'change' ,type:'string' }]
      }
    };
  },
  methods: {
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.$axios
            .post(this.$store.state.ORIGIN_API + "/api/users/register", this.registerUser)
            .then(res => {
              // 注册成功
              this.$message({
                message: "注册成功！",
